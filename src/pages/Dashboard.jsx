...
// //   Calendar, Zap, BarChart2, Activity, Sun, Moon
// // } from 'lucide-react';
// // import {
...
// //       const res = await fetch(`https://transportation-opera-replacing-flashing.trycloudflare.com/api/dashboard_stats?date=${activeDate}`);
...
// //     fetchData();
// //     let id;
// //     if (activeDate === todayStr) id = setInterval(fetchData, 5000);
// //     return () => clearInterval(id);
// //   }, [activeDate]);

// //   const handleApplyDate = () => setActiveDate(dateInput);
...
// //   const weekdayData = [
// //     { name: 'Weekday', value: weekday, fill: '#7b61ff' },
// //     { name: 'Weekend', value: weekend, fill: '#ff6b9d' },
// //   ];
...
// //             />
// //           </div>
// //           <button className="btn-apply" onClick={handleApplyDate}>Apply</button>
...
// //   const todayStr = new Date().toISOString().split('T')[0];
// //   const [activeDate, setActiveDate] = useState(todayStr);
// //   const [activeStore, setActiveStore] = useState(STORES[0].value);

// //   const fetchData = async () => {
// //     setLoading(true);
// //     try {
// //       const res = await fetch(
// //         `https://transportation-opera-replacing-flashing.trycloudflare.com/api/dashboard_stats?date=${activeDate}&store=${activeStore}`
// //       );
// //       const json = await res.json();
// //       setData(json);
// //       setLast(new Date().toLocaleTimeString());
...
// //   const activeStoreLabel = STORES.find((s) => s.value === activeStore)?.label ?? activeStore;
...
// //           </div>
// //           <p className="dash-sub">
// //             <span className="dash-sub__store-tag">{activeStoreLabel}</span>
// //             Viewing <strong style={{ color: '#8498b4' }}>{activeDate}</strong>
// //             {isToday && <span className="dash-sub-live"><span className="live-dot" />Live</span>}
// //             <span className="dash-sub__sep" />
// //             Updated: {lastUpdated ?? '--'}
...
//     if (!token) return;
//     try {
//       const res = await fetch('https://abs-towns-blocks-contacts.trycloudflare.com/api/stores', {
...
//     setLoading(true);
//     try {
//       const url = activeStore === 'All'
//         ? `https://abs-towns-blocks-contacts.trycloudflare.com/api/dashboard_stats?date=${activeDate}`
//         : `https://abs-towns-blocks-contacts.trycloudflare.com/api/dashboard_stats?date=${activeDate}&store=${activeStore}`;
...
//   const todayLegendLabel = isToday ? "Today" : "Selected Date";
//   const yesterdayLegendLabel = isToday ? "Yesterday" : "Previous Day";
//   const activeStoreLabel = activeStore === 'All' ? 'All Stores' : activeStore;
...
export default function Dashboard() {
  ...
  const fetchData = async (storeOverride) => {
    ...
    setLoading(true);
    try {
      const url = `https://abs-towns-blocks-contacts.trycloudflare.com/api/dashboard_stats?date=${activeDate}&store=${encodeURIComponent(storeToFetch)}`;
      const res = await fetch(url, {
        headers: { Authorization: `Bearer ${token}` }
      });
